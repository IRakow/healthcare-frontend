...
export default function AdminUsersPage() {
  ...
  const handleVoiceQuery = async (text: string) => {
    ...
    if (text.includes('export') || text.includes('download')) {
      exportUsers()
      speak('Exporting user list to CSV file. Download will start shortly.')
      ...
      if (searchTerm) {
        ...
        speak(`Found ${results.length} user${results.length !== 1 ? 's' : ''} matching ${searchTerm}`)
      }
      ...
      speak(`${topUser.name} has the highest impact score of ${topUser.score}`)
    } else if (user) {
      if (text.includes('deactivate') || text.includes('remove')) {
        speak(`Initiating deactivation process for ${user.name}. Please confirm in the interface.`)
        toast.error(`Deactivation initiated for ${user.email}`)
      } else {
        speak(`${user.name} is a ${user.role} with email ${user.email}. ${user.score ? `Their impact score is ${user.score}.` : ''}`)
      }
    } else {
      speak('You can ask about specific users, search for users, export the list, or find the top performer.')
    }
  }
